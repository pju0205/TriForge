...
{
	// 언리얼 로그에서 해당 HTTP 관련 데이터 타입들을 띄워주기 위한 코드
...
void FDSGameSession::Dump() const
{
	...
	UE_LOG(LogDedicatedServers, Log, TEXT("  CreationTime: %s"), *CreationTime);	// '-'이 들어가서 오류가 뜨니까 FString형으로 변환
	...
	UE_LOG(LogDedicatedServers, Log, TEXT("  TerminationTime: %s"), *TerminationTime);	// '-'이 들어가서 오류가 뜨니까 FString형으로 변환
}
